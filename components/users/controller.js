...
class Controller {
    ...
    static async getAll(req, res) {
        ...
        switch (sort) {
            case 'answers':
                ...
                break;
            default:
                aggregation.push({ $unwind: '$answered' }, {
                    $lookup: {
                        from: 'questions',
                        localField: 'answered',
                        foreignField: '_id',
                ...
                }, { $match: { 'category.name': sort } }, { $unwind: '$category' }, {
                    $group: {
                        _id: '$_id',
    ...
    static async unfollow(req, res) {
        ...
        try {
            const collection = await db.getCollection(COLLECTIONS.USERS);
            const user = await collection.updateOne({ username: username }, { $pull: { following: following } });
    ...
    static async increaseCompetitionsWon(username) {
        try {
            const collection = await db.getCollection(COLLECTIONS.USERS);
            const user = await collection.updateOne({ username: username }, { $inc: { competitions_won: 1 } });
